...
public class AutoOrientBlock : IBlock
{
    #region Fields

    private readonly IReadOnlyList<Socket> _inputs = [new("AutoOrient.In", "Image.In")];
    private readonly IReadOnlyList<Socket> _outputs = [new("AutoOrient.Out", "Image.Out")];
    ...
    private int _width;
    private int _height;
    private string _title = "Auto Orient";
    ...
    public AutoOrientBlock(int width, int height)
    {
        _width = width;
        _height = height;
    }
    ...
    [Browsable(false)]
    public string Content => "From EXIF";
        ...
        IDictionary<string, IReadOnlyList<IBasicWorkItem>> inputs, CancellationToken cancellationToken)
    {
        ...
        foreach (var sourceItem in inItems.OfType<WorkItem>())
        {
            ...
            sourceItem.Image.Mutate(x => x.AutoOrient());
