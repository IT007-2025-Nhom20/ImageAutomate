...
public class OpacityBlock : IBlock
{
    #region Fields

    private readonly IReadOnlyList<Socket> _inputs = [new("Opacity.In", "Image.In")];
    private readonly IReadOnlyList<Socket> _outputs = [new("Opacity.Out", "Image.Out")];
    ...
    public OpacityBlock(int width, int height)
    {
        _width = width;
        _height = height;
    }
    ...
    [Browsable(false)]
    public string Content => $"Opacity: {Amount * 100:F0}%";
        ...
        IDictionary<string, IReadOnlyList<IBasicWorkItem>> inputs, CancellationToken cancellationToken)
    {
        ...
        foreach (var sourceItem in inItems.OfType<WorkItem>())
        {
            ...
            {
                sourceItem.Image.Mutate(x => x.Opacity(Amount));
            }
            else
            {
                sourceItem.Image.Mutate(x => x.Opacity(1));
            } 
