...
public class InvertBlock : IBlock
{
    #region Fields

    private readonly IReadOnlyList<Socket> _inputs = [new("Invert.In", "Image.In")];
    private readonly IReadOnlyList<Socket> _outputs = [new("Invert.Out", "Image.Out")];
    ...
    private int _width;
    private int _height;
    private string _title = "Invert";

    #endregion

    public InvertBlock()
        : this(150, 80) // Compact size
    {
    }

    public InvertBlock(int width, int height)
    {
        _width = width;
        _height = height;
    }
        ...
        IDictionary<string, IReadOnlyList<IBasicWorkItem>> inputs, CancellationToken cancellationToken)
    {
        ...
        foreach (var sourceItem in inItems.OfType<WorkItem>())
        {
            ...
            sourceItem.Image.Mutate(x => x.Invert());
